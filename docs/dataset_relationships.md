...
| `normalized/endurance_score_daily.json` | daily performance context | one calendar day; `calendar_date` | direct relationship `not_yet_defined` | candidate `calendar_date` to `activity_date_local` | one context row to many Activities when resolved | normalized JSON canonical; Snapshot variants are audit-preserved when canonicalization is unresolved | standalone trends and separately labelled daily comparison; review `audit/endurance_score_daily.json` for variants | no Activity merge, causal claim, date-derived identity, or winner selection |
...
| `normalized/sleep_daily.json` | condition context | one resolved/available sleep state; `sleep_day` | same-day `context_only`; direct relationship `not_yet_defined` | `sleep_day` to `activity_date_local` | one context row to many Activities | normalized JSON canonical | compare separately aggregated same-day sleep and Activity measures using available rows only | never use `needs_review` or excluded rows for context, copy Sleep fields into Activity facts, or infer causality |
| `normalized/uds_daily.json` | condition context | one source state; `calendar_date` | same-day `context_only`; direct relationship `not_yet_defined` | `calendar_date` to `activity_date_local` | one context row to many Activities when resolved | normalized JSON canonical; Snapshot variants are audit-preserved when canonicalization is unresolved | generation-aware same-day condition comparison; review `audit/uds_daily.json` for variants | no Activity fact merge, missing-value reconstruction, or winner selection |
