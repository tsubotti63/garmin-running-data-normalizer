...
These short answers apply to Garmin Running Data Normalizer `v1.3.0` and route
...
`v1.3.0` is the current stable release on PyPI. It includes the compatible
...
v1.3.0 retains. The current release also provides the bounded
...
For a normal v1.3.0 Production PyPI installation, no separate manual
`pip install tzdata` step is expected. If the check still fails, reinstall
v1.3.0 in a new environment and include only sanitized environment details in a
support report.
