...
DATASET_TABLE = (
    ...
    {"name": "fit_sessions", "family": "fit", "record_grain": "fit_file_session", "stable_key": ("fit_file_id",), "required": False},
    {"name": "fit_laps", "family": "fit", "record_grain": "fit_file_lap", "stable_key": ("fit_file_id", "lap_index"), "required": False},
)
...
INCOMPLETE_FIT_STATUSES = {"too_large", "too_small", "bad_header", "truncated"}
...
def _validate_dataset_table() -> None:
    expected = {
        ...
        "fit_sessions": ("fit_file_session", ("fit_file_id",), False),
        "fit_laps": ("fit_file_lap", ("fit_file_id", "lap_index"), False),
    }
    actual = {
...
def _normalize_datasets(
    ...
    return records, fit_audit, {
        "incomplete_fit_count": incomplete_fit_count,
        **{f"status_{key}": value for key, value in sorted(fit_status_counts.items())},
    }
...
def _family_results(
    ...
    family_record_counts = {
        ...
        "fit": len(records["fit_sessions"]) + len(records["fit_laps"]),
    }
...
def run_all(input_path: str | Path, output_path: str | Path) -> dict[str, Any]:
    ...
    records, fit_audit, fit_status = _normalize_datasets(input_root, families)
    ...
    manifest = {
        ...
        "deterministic_output_digest": deterministic_digest,
    }
    summary = {
