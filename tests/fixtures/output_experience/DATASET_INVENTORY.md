...
| `race_prediction_daily` | source-provided race-prediction observations | `SKIPPED_NOT_PRESENT` | 0 | 0 | `normalized/race_prediction_daily.json` | source_observation | `calendar_date`, `observation_timestamp` | normalized source of truth | observation context with a non-canonical derived daily view; not an Activity fact join | `not_yet_defined` | `public-safe-metric-fields` |
...
| `training_history_daily` | limited training-status source observations | `SKIPPED_NOT_PRESENT` | 0 | 0 | `normalized/training_history_daily.json` | source_observation | `calendar_date`, `observation_timestamp` | normalized source of truth | date, timestamp, status, and optional sport context only | `not_yet_defined` | `personal-local-metric-fields` |
